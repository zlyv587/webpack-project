...
exports.default = function (_ref99) {
  ...
  return {
    visitor: {
      Program: function Program(path, _ref104) {
        var opts = _ref104.opts;
  ...
  function createStaticChecks() {
    return {
      ...
      instanceof: function _instanceof(_ref105) {
  ...
  function checkNullable(_ref107) {
    var input = _ref107.input;
    var type = _ref107.type;
  ...
  function checkStringLiteral(_ref109) {
  ...
  function checkNumericLiteral(_ref110) {
  ...
  function checkBooleanLiteral(_ref111) {
    var input = _ref111.input;
    var annotation = _ref111.annotation;
  ...
  function checkIntersection(_ref113) {
  ...
  function checkGenerator(_ref116) {
  ...
  function checkIterable(_ref117) {
  ...
  function checkTuple(_ref120) {
    var input = _ref120.input;
    var types = _ref120.types;
  ...
  function checkObject(_ref121) {
    var input = _ref121.input;
    var properties = _ref121.properties;
    var indexers = _ref121.indexers;
    ...
    var check = properties.reduce(function (expr, prop, index) {
      var target = prop.key.type === 'Identifier' ? t.memberExpression(input, prop.key) : t.memberExpression(input, prop.key, true);
    ...
    if (indexers.length) {
      return _ref34(indexers.reduceRight(function (expr, indexer) {
        if (indexer.value.type === 'AnyTypeAnnotation') {
          return expr;
        }
        var value = scope.generateUidIdentifier(indexer.id.name);
        var check = checkAnnotation(value, indexer.value, scope);
        var fixedKeys = t.arrayExpression(propNames);

        if (check) {
          if (propNames.length) {
            return t.logicalExpression('&&', expr, checkObjectIndexers({ input: input, value: value, check: check, fixedKeys: fixedKeys }));
          } else {
            return t.logicalExpression('&&', expr, checkObjectIndexersNoFixed({ input: input, value: value, check: check, fixedKeys: fixedKeys }));
          }
        } else {
          return expr;
        }
  ...
  function checkObjectPattern(_ref122) {
    var input = _ref122.input;
    var properties = _ref122.properties;
    var scope = _ref122.scope;
  ...
  function getLogicalExpressionAnnotation(path) {
    ...
    if (isBooleanExpression(node)) {
      ...
      switch (node.operator) {
        case '&&':
        case '||':
          var _ref123 = [getAnnotation(left), getAnnotation(right)];
          left = _ref123[0];
          right = _ref123[1];
